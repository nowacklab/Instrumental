    ...
    def __init__(self, *args):
        ...
        for arg in args:
            if isinstance(arg, Channel):
                channelarg.append(arg)
                namearg.append(arg.name)
            else:
                channel, name = arg
    ...
    def set_timing(self, duration=None, fsamp=None, n_samples=None,
        ...
        for typ, task in self._mxtasks.items():
            if typ == self.master_type:
                clock = bytes('', 'utf-8') #BTS 4/27/2016
            else:
                clock = bytes(master_clock, 'utf-8') #BTS 4/27/2016
...
class _Task(object):
    ...
    def config_timing(self, fsamp, samples, mode='finite', clock='',
        ...
        samples = int(samples)
        sample_mode = _sampleMode_map[mode]
        clock = bytes(clock, 'utf-8') #BTS 4/27/2016
    ...
    def _handle_di(self, di):
        ...
        elif isinstance(di, str): #BTS 4/27/2016
    ...
    def write_AO_channels(self, data, timeout=-1.0, autostart=True):
        if timeout != -1.0:
            timeout = float(Q_(timeout).to('s').magnitude)
        arr = np.concatenate([data[ao.decode('utf-8')].to('V').magnitude for ao in self.AOs]).astype(np.float64) #BTS 4/27/2016
        samples = list(data.values())[0].magnitude.size #BTS 4/27/2016
        samples_written = c_int32()
...
def list_instruments():
    ...
    dev_names = data.value.decode('utf-8').split(b',') #BTS 4/27/2016
    instruments = []
